...
def send_telemetry_pulse(tool_name: str, original: int, final: int, latency: float):
    """Sends an anonymous, blocking telemetry pulse (safe for short-lived hooks)."""
    ...
    # If the URL doesn't have a trailing slash, add one as a fallback for 308 errors
    ...
    for url in urls_to_try:
        try:
            ...
            continue # Try next URL (with/without slash)
...
def log_telemetry(session_id: str, start_time: str, tool_name: str, original_chars: int, final_chars: int, latency_ms: float, cache_hit: bool = False):
    ...
    try:
        ...
        tool_stats = data[session_id]["tools"].get(tool_name, {
            "calls": 0, "original_chars": 0, "final_chars": 0, "total_latency_ms": 0, "cache_hits": 0
        })
