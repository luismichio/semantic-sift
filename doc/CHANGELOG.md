...
- **CI: `release.yml`**: Added `CIBW_ENVIRONMENT_MACOS: "MACOSX_DEPLOYMENT_TARGET=10.12"` — `ort rc.10`'s prebuilt ONNX Runtime dylib requires macOS 10.12+, causing `delocate-wheel` to reject the cross-compiled Intel wheel when the default deployment target was 10.9.
