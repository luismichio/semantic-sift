...
*   **Routing Logic**:
    *   If `type` is `"auto"`, the tool inspects the file extension:
        *   `.log`, `.out` routes to the **Heuristic Engine** (`sift_kernel.apply_heuristic_sieve`).
        *   `.md`, `.txt`, `.rst` routes to the **Hybrid Pipeline** (`sift_kernel.perform_doc_sift`).
        *   All other extensions (including code files and general prose) route to the **Semantic Engine** (`sift_kernel.perform_semantic_sift`).
*   **Output**: The distilled file content, prepended with a Markdown Audit Header (generated by `telemetry_core.generate_audit_header`) detailing the token reduction and latency. Returns standard error strings if reading (UTF-8 or Latin-1 fallback) fails.
