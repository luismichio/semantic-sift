...
"""Hook interceptor — stdin/stdout sifting pipeline. Canonical import path."""
import sys
import json
import os
import re
...
# Import Core Logic & Telemetry via canonical paths
...
def log(message: str) -> None:
    try:
        ...
        safe_message = telemetry_core.redact_secrets_for_telemetry(message)
        ...
        if os.environ.get("CLAUDE_TOOL_NAME"):
            ...
            if "tool_args" in data:
                ...
                if platform == "Generic CLI":
                    ...
                    if (
                        "tool_response" in data
                        and "llmContent" in data["tool_response"]
                        and not os.environ.get("GEMINI_SESSION_ID")
                    ):
        ...
        with tracer.start_as_current_span("subconscious_sift") as span:
            ...
            if not sifted:
                ...
                if (is_prose or has_md_ext or is_html) and len(working_content) > 3000:
                    ...
                    semantic_output, timed_out_semantic = _semantic_sift_with_timeout(
